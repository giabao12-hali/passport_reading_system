import { ApiEtourResponse } from "../interfaces";

export const mockEtourData: ApiEtourResponse = {
    status: 200,
    message: "Success",
    code: 0,
    response: [
        {
            tourCode: "TOUR123",
            bookingID: "BOOKING001",
            bookingNo: "B123456",
            tourNote: "This is a test note 1",
            adultNumber: 2,
            childNumber: 1,
            infantNumber: 0,
            totalGuest: 3,
            memberInfors: [
                {
                    fullName: "Nguyễn Văn A",
                    gender: "Nam",
                    placeOfBirth: "Hà Nội",
                    nationality: "Việt Nam",
                    visaInfor: {
                        dateOfBirth: "01/01/1990",
                        issueDate: "01/01/2020",
                        expireDate: "01/01/2025",
                        documentNumber: "123456789"
                    },
                    idCardInfor: {
                        dateOfBirth: "01/01/1990",
                        issueDate: "01/01/2020",
                        expireDate: "01/01/2025",
                        documentNumber: "123456789"
                    },
                    address: "Hà Nội",
                    personalKind: 1,
                    birthPlace: "Hà Nội"
                }
            ]
        },
        {
            tourCode: "TOUR456",
            bookingID: "BOOKING002",
            bookingNo: "B789012",
            tourNote: "This is a test note 2",
            adultNumber: 1,
            childNumber: 2,
            infantNumber: 0,
            totalGuest: 3,
            memberInfors: [
                {
                    fullName: "Nguyễn Văn B",
                    gender: "Nữ",
                    placeOfBirth: "Hồ Chí Minh",
                    nationality: "Việt Nam",
                    visaInfor: {
                        dateOfBirth: "02/02/1992",
                        issueDate: "02/02/2020",
                        expireDate: "02/02/2025",
                        documentNumber: "987654321"
                    },
                    idCardInfor: {
                        dateOfBirth: "02/02/1992",
                        issueDate: "02/02/2020",
                        expireDate: "02/02/2025",
                        documentNumber: "987654321"
                    },
                    address: "Hồ Chí Minh",
                    personalKind: 1,
                    birthPlace: "Hồ Chí Minh"
                }
            ]
        },
        {
            tourCode: "TOUR789",
            bookingID: "BOOKING003",
            bookingNo: "B345678",
            tourNote: "This is a test note 3",
            adultNumber: 3,
            childNumber: 0,
            infantNumber: 1,
            totalGuest: 4,
            memberInfors: [
                {
                    fullName: "Nguyễn Văn C",
                    gender: "Nam",
                    placeOfBirth: "Đà Nẵng",
                    nationality: "Việt Nam",
                    visaInfor: {
                        dateOfBirth: "03/03/1993",
                        issueDate: "03/03/2020",
                        expireDate: "03/03/2025",
                        documentNumber: "567890123"
                    },
                    idCardInfor: {
                        dateOfBirth: "03/03/1993",
                        issueDate: "03/03/2020",
                        expireDate: "03/03/2025",
                        documentNumber: "567890123"
                    },
                    address: "Đà Nẵng",
                    personalKind: 1,
                    birthPlace: "Đà Nẵng"
                }
            ]
        }
    ]
};
